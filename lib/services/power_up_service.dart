...
class PowerUpService {
  ...
  PowerUpService(this._firestore);
  ...
  Future<bool> isPowerUpActive(String userId, String powerUpId) async {
    final doc = await _firestore.collection('active_power_ups').doc('${userId}_$powerUpId').get();
  ...
  Future<List<Map<String, dynamic>>> getActivePowerUps(String userId) async {
    final snapshot = await _firestore.collection('active_power_ups').where('userId', isEqualTo: userId).where('isActive', isEqualTo: true).get();
  ...
  Future<bool> purchasePowerUp(String userId, String powerUpId, int cost) async {
    return await _firestore.runTransaction((transaction) async {
      final doc = await transaction.get(_firestore.collection('users').doc(userId));
